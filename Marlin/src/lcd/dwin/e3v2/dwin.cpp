...
// JV CHANGES -----------------
#ifndef CUSTOM_LINE1
#define CUSTOM_LINE1 "Firmware by Josip Vinkovic"
#endif
#ifndef CUSTOM_LINE2
#define CUSTOM_LINE2 "www.altoden.hr"
#endif
// END JV CHANGES -----------------
...
select_t select_page{0}, select_file{0}, select_print{0}, select_prepare{0}, select_control{0}, select_axis{0}, select_temp{0}, select_motion{0}, select_tune{0}, select_PLA{0}, select_ABS{0}, select_speed{0}, select_acc{0}, select_jerk{0}, select_step{0};
...
#endif
    if (CVISI(CONTROL_CASE_INFO))
      DWIN_Draw_String(false, true, font8x16, Color_White, Color_Bg_Black, LBLX, CLINE(CONTROL_CASE_INFO), F("Info"));
#else
...
#define _TEMP_ICON(N)                                      \
  ...
  {                                                        \
    ++i;                                                   \
    if (CVISI(i))                                          \
      Draw_Menu_Line(CSCROL(i), ICON_Temperature + (N)-1); \
  } while (0)
...
inline void Draw_Info_Menu()
{
  ...
  else
  {
    ...
    // JV CHANGES -----------------
    DWIN_Draw_String(false, false, font8x16, Rectangle_Color, Color_Bg_Black, (DWIN_WIDTH - strlen(CUSTOM_LINE1) * MENU_CHR_W) / 2, 309, (char *)CUSTOM_LINE1);
    DWIN_Draw_String(false, false, font8x16, Rectangle_Color, Color_Bg_Black, (DWIN_WIDTH - strlen(CUSTOM_LINE2) * MENU_CHR_W) / 2, 336, (char *)CUSTOM_LINE2);
...
#if HAS_HEATED_BED
        if (tempbed)
          sprintf_P(cmd, PSTR("M190 S%i\n"), tempbed);
#endif
#if HAS_HOTEND
        if (temphot)
          sprintf_P(&cmd[strlen(cmd)], PSTR("M109 S%i\n"), temphot);
#endif
#endif
...
void Draw_Temperature_Menu()
{
  ...
  if (select_temp.now)
    Draw_Menu_Cursor(select_temp.now);
...
void HMI_Temperature()
{
  ...
  if (encoder_diffState == ENCODER_DIFF_CW)
  {
    if (select_temp.inc(1 + TEMP_CASE_TOTAL))
      Move_Highlight(1, select_temp.now);
  }
  else if (encoder_diffState == ENCODER_DIFF_CCW)
  {
    if (select_temp.dec())
...
inline void Draw_Max_Accel_Menu()
{
  ...
  else
  {
    ...
    say_x(108, MBASE(1)); // "Max Acceleration X"
    draw_max_accel_en(MBASE(2));
    say_y(108, MBASE(2)); // "Max Acceleration Y"
    draw_max_accel_en(MBASE(3));
    say_z(108, MBASE(3)); // "Max Acceleration Z"
#if HAS_HOTEND
    draw_max_accel_en(MBASE(4));
    say_e(108, MBASE(4));                      // "Max Acceleration E"
#endif
#endif
  }
...
inline void Draw_Steps_Menu()
{
  ...
  else
  {
    ...
    say_x(103, MBASE(1)); // "Steps-per-mm X"
